...
- Keychain account convention, pinned here because the merged code currently disagrees with itself (FillShell uses `url.lastPathComponent` INCLUDING the extension; CLI and MCP use `deletingPathExtension().lastPathComponent`): the standard per-file account is `deletingPathExtension().lastPathComponent`, bare, with NO edge prefixes. Implementation found the divergence ran deeper than the extension: the CLI wrapped accounts as "lda-<base>" and MCP as "ai.openclaw.lda.mcp.profile.<base>", so Keychain files were never interoperable across edges. Saves (UI export, CLI, MCP) now all use the bare standard account. Loads try the standard account first and fall back to the three legacy formats (UI extension-included, CLI "lda-" prefixed, MCP prefixed), retained for reads only, so files saved by any pre-portal build still open. The UI save path is corrected to the standard convention as part of this work. The library's own files do not use per-file accounts at all; they use a single library account constant `library` under the profile service.
