...
1. Open LDA and choose or import a compatible local detection model. Patterns only has limited coverage and does not detect names, companies or addresses.
...
New source builds include both videos in the first-run workflow page and **Settings > Tutorials**. Setup-enabled builds also show them in **MCP Setup**. Open a video and press Play; closing the player stops playback. A transcript is available below the player.
