//
//  FillServiceApplyTests.swift
//  LDACoreTests
//
//  Tests for LDAService.applyFill: the DOCX and AcroForm happy paths, skipped
//  blanks (unmatched/rejected), stale target detection, duplicate confirmed
//  blanks, model load failure propagation, kind propagation, and re-fill
//  collision avoidance. All tests are hermetic: fixtures are generated under
//  FileManager.temporaryDirectory; no binaries are committed.
//
//  Duplicated from FillServiceTests (deliberate: each test file is intentionally
...
final class FillServiceApplyTests: XCTestCase {
    ...
    /// FillServiceTests; each test file is intentionally self-contained.
    ...
    private func writeFixtureDocx(_ paragraphs: [FixtureParagraph], name: String? = nil) throws -> URL {
        let fileName = name ?? "lda-fillsvcapply-\(UUID().uuidString).docx"
    ...
    private func makeFormPDF() throws -> URL {
        ...
        guard document.write(to: url) else {
            throw NSError(domain: "FillServiceApplyTests.fixture", code: 1)
        }
        return url
    }
