...
final class CLITests: XCTestCase {
    ...
    // MARK: - Portfolio CLI tests (Task 7)
    //
    // Tests that touch PortfolioLibrary use a temp-root library (Keychain-backed).
    // They share the same keychain skip probe used by PortfolioLibraryTests: probe
    // the index container service before attempting any library operation, and skip
    // the whole block when the Keychain is unavailable in this process.
    //
    // The probe is inlined in each affected test rather than in setUp so that the
    // non-library tests (parsing, validate, etc.) remain runnable without Keychain
    // access.
    ...
    private func keychainUnavailable() -> Bool {
        ...
        let probeAccount = "cli-tests-probe-\(UUID().uuidString)"
