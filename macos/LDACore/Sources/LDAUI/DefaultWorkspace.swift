...
enum DefaultWorkspace {
    ...
    /// KNOWN TRADE, recorded rather than quietly accepted.
    ///
    /// The stem is in the file name, so a listing of the workspaces directory
    /// is a de facto matter index in a predictable place. In PRC practice a
    /// document's file name IS the party's name, so that is a real
    /// confidentiality cost, and it breaks this project's own convention:
    /// session records are named by UUID (SessionRecordStore) precisely so a
    /// listing says nothing.
    ///
    /// It is still the stem, for a reason worth knowing before changing it.
    /// The stem is load bearing: unambiguousCandidate(forRedactedFileNamed:)
    /// finds this file from the redacted document's name alone, which is the
    /// only thing Restore has to go on. Making the name opaque therefore needs
    /// a DERIVABLE opaque name, and the obvious primitive here,
    /// SecurityEventLog.subjectDigest, cannot serve: its key falls back to an
    /// ephemeral per-process key when the Keychain will not store one, so a
    /// name minted at export would not be recomputable at restore and the
    /// mapping would become unfindable. That is the worst outcome this product
    /// has, traded for a smaller one.
    ///
    /// A real fix needs a digest key that is persistent or the export refuses,
    /// which is a new way for an export to fail and a decision about the
    /// user's threat model rather than a defect to patch. Left as it is,
    /// deliberately, with the cost stated.
